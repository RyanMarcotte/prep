...
namespace prep.specs
{
  ...
  public class MovieLibrarySpecs
  {
    ...
    public class when_searching_for_movies : concern_for_searching_and_sorting
    {
      ...
      {
        // > 2004
        var criteria = Where<Movie>.has_an(x => x.date_published.Year).falls_in(new RangeBuilder<int>(int.MinValue, int.MaxValue).greater_than(2004).range());
      ...
      {
        //1982-2003 - inclusive
        var criteria = Where<Movie>.has_an(x => x.date_published.Year).falls_in(new RangeBuilder<int>(int.MinValue, int.MaxValue).greater_than_or_equal_to(1982).less_than_or_equal_to(2003).range());
